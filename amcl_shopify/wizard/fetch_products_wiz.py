...
class ProductsFetchWizard(models.Model):
                                                ...
                                                if prod.product_template_attribute_value_ids[0].name == child.get(
                                                    ...
                                                    if child.get('barcode'):
                                                        barcodes = self.env['product.product'].search([]).mapped(
                                                            'barcode') + self.env['product.template'].search([]).mapped(
                                                            'barcode')
                                                    ...
                                                    prod.write({
                                                        ...
                                                        'barcode': barcode,
                                                        'shopify_type': 'simple',
                                                        'image_1920': self.shopify_image_processing(
                                                            child_file) if marketplace_instance_id.sync_product_image == True else False,
    ...
    def shopify_fetch_products_to_odoo(self, kwargs):
        ...
        categ_list = []
        print('Lenght :', len(kwargs.get('marketplace_instance_id')))
        ...
        try:
            ...
            # config_prod_list = configurable_products['products']F
