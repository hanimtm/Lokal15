...
class OrderFetchWizard(models.Model):
    ...
    def shopify_fetch_orders(self, kwargs=None):
        ...
        url = marketplace_instance_id.marketplace_host + \
              "/admin/api/2022-01/orders.json?status=any"
            ...
            for i in sp_orders:
                if str(i['id']) not in order_ids and i['confirmed'] == True:
                    ...
                    if marketplace_instance_id:
                        # Options
                        order_vals[
                            'warehouse_id'] = marketplace_instance_id.warehouse_id.id if marketplace_instance_id.warehouse_id else None
                        order_vals[
                            'company_id'] = marketplace_instance_id.company_id.id if marketplace_instance_id.company_id else None
                        order_vals[
                            'user_id'] = marketplace_instance_id.user_id.id if marketplace_instance_id.user_id else self.env.user.id
                        order_vals['fiscal_position_id'] = marketplace_instance_id.fiscal_position_id.id or None
                        order_vals[
                            'pricelist_id'] = marketplace_instance_id.pricelist_id.id if marketplace_instance_id.pricelist_id else None
                        order_vals[
                            'payment_term_id'] = marketplace_instance_id.payment_term_id.id if marketplace_instance_id.payment_term_id else None
                        order_vals[
                            'team_id'] = marketplace_instance_id.sales_team_id.id if marketplace_instance_id.sales_team_id else None
                    ...
                    for line in i['line_items']:
                        product_tax_per = 0
                        product_tax_name = ''
                        print('Default Code :: ', str(line['sku']))
                        ...
                        if not prod_rec and marketplace_instance_id.auto_create_product and line.get(
                        ...
                        product_tax = self._shopify_get_taxnames(
                            line['tax_lines'])
                        _logger.info("product_tax===>>>>>" + str(product_tax))
                    ...
                    order_vals[
                        'partner_shipping_id'] = partner_shipping_id.id if partner_shipping_id != False else pp.id
                    ...
                    if order_vals.get('order_line'):
                        ...
                        if not product_missing and order_vals['partner_id']:
                            ...
                            try:
                                if order_id and order_id.state in ['sale',
                                                                   'done'] and marketplace_instance_id.auto_create_invoice == True:
                                    ...
                                    _logger.info("Unable to create Invoice for order id: %s" % (order_id))
                            except Exception as e:
                                _logger.warning(
                    ...
                    if marketplace_instance_id.auto_create_invoice == True:
                        print("current_order_id===>>>" + str(current_order_id.name))
    ...
    def _get_delivery_line(self, i, order_vals, marketplace_instance_id):
        ProductObj = self.env['product.product'].sudo()
        service = self.env.ref('amcl_shopify.shopify_shipping')
    ...
    def process_discount_codes(self, sp_order, order_vals):
        ...
        service = self.env.ref('amcl_shopify.shopify_discount')
    ...
    def _create_invoice_shopify(self, order_id, sp_order):
        print('Started the invoice for :: ', order_id)
        mkplc_id = self._get_instance_id()
        # move_id = order_id._create_invoices_new(mkplc_id)
        move_id = self.create_sale_bill(order_id)
        self.create_vendor_bill(order_id,mkplc_id)
        move_id.update({'marketplace_type': 'shopify', })
        return move_id

    def create_sale_bill(self, order_id):
        moves = self.env['account.move']
        for order in order_id:
            order = order.with_company(order.company_id)
            invoice_vals = self._sale_prepare_invoice(order)
            invoice_vals_list = []
            for sale_lines in order_id.order_line:
                invoice_vals['invoice_line_ids'].append((0, 0, self._prepare_sale_move_line(sale_lines, order)))
                invoice_vals['ref'] = order.name
                invoice_vals['marketplace_type'] = 'shopify'
            ...
            for vals in invoice_vals_list:
                if not self.env['account.move'].search(
                        [('ref', '=', order.name), ('partner_id', '=', order_id.partner_id.id)]):
                    invoice = AccountMove.with_company(self.env.company.id).create(vals)
                    moves |= invoice
        return moves

    def _sale_prepare_invoice(self, order):
    ...
    def create_vendor_bill(self, order_id ,mkplc_id):
        moves = self.env['account.move']
        for order in order_id:
            order = order.with_company(order.company_id)
            grouped_po_lines = groupby(order.order_line.sorted('product_id'), key=lambda l: l.shopify_vendor)
            for vendor, po_lines in grouped_po_lines:
                if vendor:
                    invoice_vals_list = []
                    invoice_vals = self._vendor_prepare_invoice(vendor, order)
                    for line in po_lines:
                        invoice_vals['invoice_line_ids'].append(
                            (0, 0, self._prepare_vendor_move_line(line, order, vendor ,mkplc_id)))
                        invoice_vals['ref'] = order.name
                        invoice_vals_list.append(invoice_vals)
                    AccountMove = self.env['account.move'].with_context(default_move_type='in_invoice')
                    for vals in invoice_vals_list:
                        if not self.env['account.move'].search(
                                [('ref', '=', order.name), ('partner_id', '=', vendor.id)]):
                            invoice = AccountMove.with_company(self.env.company.id).create(vals)
                            moves |= invoice

    def _vendor_prepare_invoice(self, vendor, order):
        ...
        partner_invoice_id = vendor.address_get(['invoice'])['invoice']
        invoice_vals = {
            'ref': order.name or '',
            'move_type': move_type,
            'currency_id': order.currency_id.id,
    ...
    def _prepare_vendor_move_line(self, line, move, vendor, mkplc_id):
        self.ensure_one()
        aml_currency = move and move.currency_id or self.currency_id
        date = fields.Date.today()
        if vendor.shopify_commission > 0:
            commission = (move.currency_id._convert(line.price_subtotal, aml_currency, self.env.company.id, date,
                                                    round=False)) * (vendor.shopify_commission / 100)
        elif mkplc_id.global_commission > 0:
            commission = (move.currency_id._convert(line.price_subtotal, aml_currency, self.env.company.id, date,
                                                    round=False)) * (mkplc_id.global_commission / 100)
        else:
            commission = move.currency_id._convert(line.price_subtotal, aml_currency, self.env.company.id, date,
                                                   round=False)
        res = {
